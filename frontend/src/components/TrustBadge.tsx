...
const HEALTH_CONFIG = {
  excellent: { color: 'text-emerald-400', bar: 'from-emerald-500 to-emerald-400', bg: 'bg-emerald-400/10 border-emerald-400/20', icon: <Shield size={14} className="text-emerald-400" /> },
      ...
      <div className={clsx('glass-card p-3 border', config.bg)}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            {config.icon}
            <span className="text-xs font-semibold text-white">Trust Balance</span>
          </div>
        ...
        <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
          <motion.div
            className={clsx('h-full rounded-full bg-gradient-to-r', config.bar)}
            initial={{ width: '100%' }}
            animate={{ width: `${pct}%` }}
        ...
        <div className="flex justify-between mt-1.5">
          <span className={clsx('text-xs font-medium capitalize', config.color)}>
            {trustHealth.replace('_', ' ')}
          </span>
      ...
      <AnimatePresence>
        {trustCredit && (
          <motion.div
            ...
            className="glass-card p-3 border border-emerald-500/30 bg-emerald-500/5"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-start gap-2">
                <Gift size={14} className="text-emerald-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-emerald-400">Credit Issued!</p>
                  <p className="text-xs text-gray-300 mt-0.5">₹{trustCredit.inr_value} credit</p>
                  <code className="text-xs font-mono text-emerald-300 bg-emerald-900/30 px-1.5 py-0.5 rounded mt-1 block">
